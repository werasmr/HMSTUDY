      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-12 text-center">
          ...
          <p className="mt-3 text-muted-foreground">
            7 модулей автоматизации бизнеса в одном месте — кликните на карточку для подробностей
          </p>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {FEATURES.map((feature, index) => (
            <Reveal key={feature.id} delay={index * 70}>
              <button type="button" onClick={() => setSelected(feature)} className="w-full text-left">
                <Card
                  className={cn(
                    "h-full transition-all duration-300 hover:-translate-y-1.5 hover:shadow-lg hover:shadow-primary/10 hover:border-primary/30",
                    "cursor-pointer",
                  )}
                >
                  <CardHeader>
